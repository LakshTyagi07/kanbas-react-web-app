...
const Dashboard = ({ courses, course, setCourse, addNewCourse, deleteCourse, updateCourse }: {
  ...
  console.log("Courses prop received:", courses);
  
  const handlePublish = async (courseId: string) => {
    try {
      const updatedCourse = await courseClient.publishCourse(courseId);
      ...
      <div id="wd-dashboard-courses" className="row">
        <div className="row row-cols-4 row-cols-xs-1 row-cols-sm-2 row-cols-lg-3 row-cols-xl-4 row-cols-xxl-5 g-4">
          {courses.map((course, index) => {
            return (
              <div className="wd-dashboard-course" key={`${course._id}-${index}`}>
                      ...
                      <button className="btn btn-primary">Go</button>
                        ...
                        {currentUser.role === "FACULTY" && (
                          <>
                            ...
                            <button 
                              className={`btn ${course.published ? 'btn-success' : 'btn-warning'}`}
